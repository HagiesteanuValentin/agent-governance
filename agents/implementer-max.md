...
description: The debugging implementer (Opus 5.5 high) — ONLY debugging declared at plan time, with a written reason. Not a default and not the escalation: `implementer` (low) covers every brief; `implementer-complex` (medium) is the plan-time choice for cross-file logic and the 3rd run after 2 DEVIATIONS.
