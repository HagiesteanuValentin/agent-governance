...
description: The implementer for HARD tasks — large refactors, difficult debugging, a sprawling definition of done. Identical to `implementer` but with high effort; pick it only with the reason named in the brief. For everything else use `implementer`.
model: opus
effort: high
maxTurns: 200
permissionMode: auto
