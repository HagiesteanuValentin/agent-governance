...
- `main_read_files`: Bash calls in main that read files (`cat`, `sed -n`, `head`, `tail`,
  `less`, long heredocs, big `grep`/`wc` output) instead of delegating the read.
- `narration_turns`: API calls in main with no tool use and short text, i.e. narration
  between waits rather than a single line per step.
...
- A `postmortem` object per session: hands-on ratio (direct tool use vs. delegation),
  narration call count, and wasted tokens as a percentage of main's input volume.
...
workflow change, and sessions before the first version are `older`.
