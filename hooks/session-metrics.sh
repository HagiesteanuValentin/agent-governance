...
SID=$(printf '%s' "$INPUT" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("session_id","session"))' 2>/dev/null)
[ -n "$TP" ] && [ -f "$TP" ] && [ -f "$TOOL" ] || exit 0
mkdir -p "$OUT_DIR"
python3 "$TOOL" "$TP" --json --out "$OUT_DIR/${SID}.json" 2>/dev/null
exit 0
