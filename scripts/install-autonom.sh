#!/bin/bash
# 🔴 rulat manual de Vali (clasificatorul auto mode blochează agenții pe ~/.claude) — docs/DECIZII.md «Mod autonom (04.09.2026)»
set -eu
...
hunk='# 🔴 marker autonom-<sid> = fără ask — docs/DECIZII.md «Mod autonom (04.09.2026)»\nif os.path.exists(os.path.join(MARKER_DIR, "autonom-%s" % safe_sid)):\n    sys.exit(0)\n'
i=t.rindex('rows = read_rows()'); open(p,'w').write(t[:i]+hunk+t[i:])
PY
python3 - "$LIVE/settings.json" <<'PY'
import json,sys; p=sys.argv[1]; s=json.load(open(p)); h=s['hooks']; H=lambda m:{"type":"command","command":"/home/vali/.claude/hooks/autonom.sh "+m}
...
PY
python3 -c "import json;json.load(open('$LIVE/settings.json'))"; echo "autonom.sh in settings: $(grep -c autonom.sh "$LIVE/settings.json") (aștept 2)"
diff <(sed 's/#.*//' "$REPO/hooks/agenti-vii.sh") <(sed 's/#.*//' "$LIVE/hooks/agenti-vii.sh") && echo "agenti-vii live = repo (cod)"
